...
public class SpectatorHandlerInterceptorIntegrationTests {
	@Autowired
	...
	public void metricsGatheredWhenSuccess() throws Exception {
		...
		assertTimer("test_some_request_-id-", "none", 200, "testSomeRequest");
	}
	...
	public void metricsGatheredWhenClientRequestBad() throws Exception {
		...
		assertTimer("test_some_request_-id-", "none", 400, "testSomeRequest");
	}
	...
	public void metricsGatheredWhenUnhandledError() throws Exception {
		...
		assertTimer("test_some_unhandledError_-id-", "RuntimeException", 200, "testSomeUnhandledError");
	}
	...
	public void metricsGatheredWhenHandledError() throws Exception {
		...
		assertTimer("test_some_error_-id-", "none", 422, "testSomeHandledError");
	}

	protected void assertTimer(String uriTag, String exceptionType, Integer status, String handlerName) {
		assertEquals(
				1,
				registry.timer("metricName", "method", "GET", "uri", uriTag, "handlerName", handlerName, "caller",
						"unknown", "exceptionType", exceptionType, "status", status.toString(), "bucket", "01s")
						.count());
	}
}
