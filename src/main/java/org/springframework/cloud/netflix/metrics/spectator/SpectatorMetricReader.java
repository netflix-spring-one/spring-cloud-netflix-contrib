...
/**
 * Reads metrics from Spectator, placing Spectator tags in the form <code>name(k1=v1,k2=v2)</code> where
 * <code>name</code> is the metric name and the contents of the parentheses are tag key-value pairs.
 *
...
public class SpectatorMetricReader implements MetricReader {
	...
	protected static String asHierarchicalName(Id id) {
		List<String> tags = stream(id.tags().spliterator(), false).map(t -> t.key() + "=" + t.value()).collect(
				Collectors.toList());
	...
	@Override
	public long count() {
		return stream(registry.spliterator(), false).flatMap(m -> stream(m.measure().spliterator(), false)).count();
	}
}
